...
export async function screenToken(contractAddress, chain, etherscanKey) {
    ...
    if (creationData) {
        if (creationData.deployer)
            deployerAddress = creationData.deployer;
        contractAgeHours = Math.max(0, Math.round((Date.now() - creationData.timestamp) / 3600000));
    }
    else if (dexData?.pairCreatedAt) {
        // Fallback: use pair creation time from DexScreener as a proxy
        contractAgeHours = Math.max(0, Math.round((Date.now() - dexData.pairCreatedAt) / 3600000));
    }
    ...
    const lockDurationDays = null; // Requires on-chain locker ABI — v2 feature
    ...
    if (isHoneypot)
        riskFlags.push("HONEYPOT DETECTED — tokens cannot be sold");
    if (ownerCanChangeBalance)
        riskFlags.push("Owner can arbitrarily modify wallet balances");
    ...
    if (sniperHeldPercent > THRESHOLDS.HIGH_SNIPER_PERCENT) {
        riskFlags.push(`${sniperHeldPercent}% of early buyers are fresh wallets (sniper pattern)`);
    }
    // ── Risk Scoring ──────────────────────────────────────────────────────────
    ...
    const summary = buildSummary(tokenName, tokenSymbol, riskScore, riskFlags, liquidityUsd, contractAgeHours, snipers.length, earlyBuyers.length);
    return {
...
function computeRiskScore(inputs) {
    const { isHoneypot, ownerCanChangeBalance, sellTax, sniperHeldPercent, deployerFlagged, riskFlagCount, liquidityLocked, } = inputs;
    // Hard fails → immediate LIKELY_RUG regardless of other signals
    if (isHoneypot)
        return "LIKELY_RUG";
    if (ownerCanChangeBalance)
        return "LIKELY_RUG";
    if (sellTax > THRESHOLDS.EXTREME_TAX_PERCENT)
        return "LIKELY_RUG";
    if (sniperHeldPercent > THRESHOLDS.EXTREME_SNIPER_PERCENT && deployerFlagged)
        return "LIKELY_RUG";
    // Soft fails → CAUTION
    if (riskFlagCount >= THRESHOLDS.FLAGS_FOR_CAUTION)
        return "CAUTION";
    if (!liquidityLocked)
        return "CAUTION";
    if (sniperHeldPercent > THRESHOLDS.HIGH_SNIPER_PERCENT)
...
function buildSummary(name, symbol, score, flags, liquidity, ageHours, sniperCount, buyerCount) {
...
//# sourceMappingURL=screen_token.js.map