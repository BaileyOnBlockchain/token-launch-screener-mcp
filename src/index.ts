...
// ─── Result Cache ─────────────────────────────────────────────────────────────
// Module-level so it survives across the per-request McpServer instances.
// Keyed by "<address_lower>:<chain>". Capped at 100 entries; oldest evicted first.

interface CachedScreenResult {
  structured: {
    ...
    flags:              string[];
    summary:            string;
    goplus_source:      string;
    dexscreener_source: string;
    etherscan_source:   string;
  };
  cached_at: string; // ISO 8601
}

const resultCache = new Map<string, CachedScreenResult>();
const CACHE_MAX = 100;

function cacheSet(address: string, chain: string, entry: CachedScreenResult): void {
...
function cacheGet(address: string, chain: string): CachedScreenResult | undefined {
...
SKIP WHEN: the token has not been screened yet (cache miss → call screen_new_token instead).`,
    ...
    async (params: any) => {
      ...
      if (!cached) {
        ...
        return {
          content: [{ type: "text" as const, text: miss.summary }],
      ...
      return {
        content: [{ type: "text" as const, text: `[Cached ${cached.cached_at}]\n\n${cached.structured.summary}` }],
